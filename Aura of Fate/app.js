...
function initialize() {
  ...
  state.deck = [...data.majorCards.cards, ...data.minorCards.minor_arcana_cards].map(card => ({ id: card.id, nameZh: card.name_zh, nameEn: card.name_en, imageFile: card.image_file, keywords: card.keywords, coreMeaning: card.core_meaning, interpretation: state.interpretationsById.get(String(card.id)) }));
...
function renderReading() {
  ...
  elements.cardReadings.innerHTML = state.draw.map(item => { const keywords = item.card.keywords?.[item.orientation] || []; return `<article class="reading-card"><p class="kicker">${escapeHtml(item.position)}・${item.orientation === "upright" ? "正位" : "逆位"}</p><h3>${escapeHtml(item.card.nameZh)}</h3><p>${escapeHtml(localMeaning(item))}</p>${keywords.length ? `<p class="card-keywords">關鍵字：${escapeHtml(keywords.join("、"))}</p>` : ""}</article>`; }).join("");
...
function buildLocalAnalysis() { const cards = state.draw.map(item => { const orientation = item.orientation === "upright" ? "正位" : "逆位"; const keywords = item.card.keywords?.[item.orientation] || []; return `<article class="local-analysis-item"><h4>${escapeHtml(item.position)}｜${escapeHtml(item.card.nameZh)}（${orientation}）</h4><p>${escapeHtml(localMeaning(item))}</p>${keywords.length ? `<p class="local-analysis-keywords">關鍵字：${escapeHtml(keywords.join("、"))}</p>` : ""}</article>`; }).join(""); return `<p class="local-reading-note">以下內容由內建牌義資料整理，完全在瀏覽器本機完成。</p><div class="local-analysis-list">${cards || "<p>尚未抽出牌卡。</p>"}</div>`; }
